...
function checkWinCondition() {
  var _ref = [boardState[0][0], boardState[0][1], boardState[0][2], // Top row: left, middle, right
  ...
  if ( // horizontal wins
  tl != "" && tl == tm && tm == tr || ml != "" && ml == mm && mm == mr || bl != "" && bl == bm && bm == br || // vertical wins
  tl != "" && tl == ml && ml == bl || tm != "" && tm == mm && mm == bm || tr != "" && tr == mr && mr == br || // diagonal wins
  tl != "" && tl == mm && mm == br || tr != "" && tr == mm && mm == bl) {
    gameOver = true;
    console.log("".concat(currentMove, " WINS!"));
  } else if (moveCount >= 8) {
    gameOver = true;
    console.log("Tie!");
  }
